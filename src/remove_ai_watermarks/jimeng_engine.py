...
class JimengEngine:
    ...
    def _locate_corner(self, image: NDArray[Any], corner: str) -> JimengLocation:
    ...
    def detect(self, image: NDArray[Any]) -> JimengDetection:
        ...
        if not all_dets:
            return JimengDetection()
        return max(all_dets, key=lambda d: d.confidence)

    def detect_all(self, image: NDArray[Any]) -> list[JimengDetection]:
        """Detect visible Jimeng marks in ALL corners that exceed the NCC threshold.
        ...
        for corner in self._all_corners():
            ...
            if coverage >= DETECT_MIN_COVERAGE:
                ...
                logger.debug("Jimeng detect %s: coverage=%.3f ncc=%.2f", corner, coverage, score)
