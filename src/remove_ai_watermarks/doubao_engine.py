...
class DoubaoEngine:
    ...
    def locate(self, image: NDArray[Any]) -> DoubaoLocation:
        ...
        h, w = image.shape[:2]
        return self._locate_corner(image, "bottom-right")

    def _locate_corner(self, image: NDArray[Any], corner: str) -> DoubaoLocation:
        ...
        margin_x = max(4, int(w * self.margin_right_frac))
        margin_y = max(4, int(w * self.margin_bottom_frac))
    ...
    def _infer_corner_from_region(self, image: NDArray[Any], region: tuple[int, int, int, int]) -> None:
        ...
        Called by the registry before remove when a specific detection region is given
        (multi-mark removal path).
        """
        h, w = image.shape[:2]
        rx, ry, rw, rh = region
        cx = rx + rw / 2.0
    ...
    def detect(self, image: NDArray[Any]) -> DoubaoDetection:
        ...
        Scans all four corners and returns the best match. Keys on the "豆包AI生成" SHAPE, not coverage,
        so a textured corner does not fire. ``confidence`` is the correlation score;
        ``detected`` is it clearing ``DETECT_NCC_THRESHOLD``.
        """
        all_dets = self.detect_all(image)
        if not all_dets:
            return DoubaoDetection()
        return max(all_dets, key=lambda d: d.confidence)

    def detect_all(self, image: NDArray[Any]) -> list[DoubaoDetection]:
        """Detect visible Doubao marks in ALL corners that exceed the NCC threshold.
        ...
        for corner in self._all_corners():
            ...
            if coverage >= DETECT_MIN_COVERAGE:
                ...
                logger.debug("Doubao detect %s: coverage=%.3f ncc=%.2f", corner, coverage, score)
    ...
    def _fixed_alpha_map(self, image: NDArray[Any]) -> tuple[NDArray[Any], tuple[int, int, int, int]] | None:
        ...
        margin_x = int(_ALPHA_MARGIN_RIGHT_FRAC * w)
    ...
    def inpaint_force_fallback(self, image: NDArray[Any], region: tuple[int, int, int, int]) -> NDArray[Any]:
        ...
        """
        h, w = image.shape[:2]
        # Use _locate_corner() + extract_mask() on the already-processed image to find
        # remaining bright watermark pixels, targeting the same corner detect() found.
        loc = self._locate_corner(image, self._last_detected_corner)
        mask = self.extract_mask(image, loc)
        x, y, bw, bh = loc.bbox
        box = mask[y : y + bh, x : x + bw]
